...
% tic;
 ...
 len1 = size(files,1);
  for k=1:len1;
img2  = imread(strcat(pet_path,char(files(k).name)));   %anatomical image
img1  = imread(strcat(mr_path,char(files_mr(k).name))); %functional image
% figure;imshow(img1);
% figure;imshow(img2);
% tic;
...
imwrite(uint8(imgf_RGB*255),strcat('F:\文章1\Medical-Image-Fusion-master\baseline\liuYu\CNN_medical_image_fusion\result\pet-mr\',files(k).name(end-6:end-4),'.png'));
  end
