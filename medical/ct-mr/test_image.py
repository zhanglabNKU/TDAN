...
from net_dcd import DenseFuse_net,DY_MedFuse_net
...
def load_model(path, input_nc, output_nc):
	...
	nest_model = DY_MedFuse_net(pretrained_model,None,input_nc, output_nc)

	# nest_model.load_state_dict(torch.load(path)['model'])
...
##multi-focus 大小为520*520
	...
	##medical image
	if 'ct' in  infrared_path:
		# file_name = 'fusion_' + fusion_type + '_' + str(index) +  '_network_' + network_type + '_' + strategy_type + '_' + ssim_weight_str + "_"+infrared_path[-11:-4] + '.png'
		file_name = 'fusion_' + fusion_type + '_' + str(index) + ssim_weight_str + "_"+infrared_path[-11:-4] + '_decoder1.png'

	else:
		file_name = 'fusion_' + fusion_type + '_' + str(index) + ssim_weight_str + "_"+infrared_path[-9:-4] + '.png'
		# file_name = 'fusion_' + fusion_type + '_' + str(index) + ssim_weight_str + "_" +'pet-mr' + '.jpg'
...
def main():
	...
	# test_path = "/home/hww/fusion0820/data/valid/"##ct-mr
	# test_path = "/home/hww/fusion0820/data/case6/"
	test_path = "/home/hww/fusion0820/MF_dataset/Code/rgb_data/case22/"##pet
	...
	# output_path = '/home/hww/fusion0820/20210421/dcd_encoder+decoder/models/1e0/'
	output_path = '/home/hww/fusion0820/20210421/dcd_encoder+decoder/models/1e0/123/pet/'
	...
	##增大图像大小
		...
		# 	if lists.endswith('.png') and lists.find('dg1_0')!=-1:
		# 		pet_path = test_path + lists
		# 		img = cv2.imread(pet_path, -1)
		# 		height, width = img.shape[:2]
		# 		fx = 2
		# 		fy = 2
		# 		enlarge = cv2.resize(img, (0, 0), fx=fx, fy=fy, interpolation=cv2.INTER_CUBIC)
		# 		enlarge = enlarge[:, :, :: -1]
		# 		enlarge = Image.fromarray(enlarge)
		# 		enlarge.save(pet_path[:-7]+'_enlarge_'+pet_path[-7:-4]+'.png')
	...
	for lists in listdir(test_path):##一个model
		if lists.endswith('.png') and lists.find('dg1__en')!=-1:
			...
			# pet_path = '/home/hww/fusion0820/MF_dataset/Code/rgb_data/case22/dg1_013.png'
			...
			y_path = '/home/hww/fusion0820/20210421/dcd_encoder+decoder/models/1e0/pet_y_channel/' + "y_" + pet_path[-7:-4] + '.png'
			im.save(y_path)

			cr = Image.fromarray(ycrcb_image[:, :, 1])
			cb = Image.fromarray(ycrcb_image[:, :, 2])
			cr.save('/home/hww/fusion0820/20210421/dcd_encoder+decoder/models/1e0/pet_y_channel/' + "cr_" + pet_path[-7:-4] + '.png')
			cb.save('/home/hww/fusion0820/20210421/dcd_encoder+decoder/models/1e0/pet_y_channel/' + "cb_" + pet_path[-7:-4] + '.png')
		...
		# 		run_demo(model, infrared_path, visible_path, output_path, index, fusion_type, network_type, strategy_type, ssim_weight_str, mode)
		# 	if lists.endswith('.png') and lists.find('ct1')!=-1:
		# 		infrared_path = test_path + lists
